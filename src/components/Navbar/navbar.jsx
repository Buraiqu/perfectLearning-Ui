...
const CustomNavbar = ({user}) => {
    ...
    const handleSearchChange = () => {
        ...
        if (searchQuery.trim()) {
            // Only perform search if there's actual text in the search query
            // Add your search logic here
        }
    };
    
    // Effect to handle clearing search results when search query is empty
    useEffect(() => {
        if (searchQuery === '') {
            // Clear search results when search query is empty
            console.log('Search query cleared, resetting results');
            // Add your logic to clear search results here
        }
    }, [searchQuery]);
                    ...
                    <Form className="d-flex align-items-center search-form me-4 me-auto">
                        <div className="search-container">
                            <Form.Control
                                type="search"
                                placeholder="Search"
                                className="search-input no-clear-button"
                                aria-label="Search"
                                value={searchQuery}
                                onChange={(e) => setSearchQuery(e.target.value)}
                            />
                            <div className="search-icon" onClick={handleSearchChange}>
